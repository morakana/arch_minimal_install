...
ArchLinuxのインストーラ( https://archlinux.org/download/ から取得)を起動します
...
./phase01.sh として フェーズ1を起動します
  ...
  --------------------------------------------------------------
  lsblkを実行してみて、
  システムパーティション(/にあたる部分)が /mnt
  EFIシステムパーティション(ESP) が /mnt/boot
  にマウントされていれば大丈夫です。
  システムパーティションをマウント後、
  mkdir /mnt/boot とマウントポイントを作ってから、
  ESPを /mnt/boot にマウントする感じでやれば大丈夫です。
  データ用のパーティション があるなら、この時点で任意の場所にマウントしておけば、
  インストール完了後もその構成が維持されます (新しい環境の /etc/fstabに反映されます)。
  
  gdisk /dev/＜対象ドライブ sdaなど＞                 <- パーティション管理 (ESPはEF00、システムやデータは8300)
  mkfs.fat -F32 /dev/＜対象パーティション sda1など＞   <- ESPのフォーマット
  mkfs.ext4 /dev/＜対象パーティション sda2など＞       <- システムやデータパーティションのフォーマット
  mount /dev/＜対象パーティション sda2など＞ /mnt      <- システムパーティションをマウント
  mkdir /mnt/boot                                  <- ESPのマウントポイントの作成
  mount /dev/＜対象パーティション sda2など＞ /mnt/boot <- ESPをマウント
  --------------------------------------------------------------
  これでフェーズ1 は終了です。
  --------------------------------------------------------------
...
./phase03.sh として フェーズ3を起動します
  ...
  あとは、指示のとおりに
  exit で chrootから抜けて
  ArchLinuxのインストールメディアを取り出し再起動します
  --------------------------------------------------------------
  これで全てのフェーズが完了です。
  --------------------------------------------------------------
